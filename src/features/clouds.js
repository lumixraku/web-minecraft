...
export function generateClouds() {
  ...
  for (let i = 0; i < CLOUD_COUNT; i++) {
    const cx = Math.floor((Math.random() - 0.5) * SIZE * 1.4);
    const cz = Math.floor((Math.random() - 0.5) * SIZE * 1.4);
    const cy = CLOUD_BASE_Y + Math.floor(Math.random() * 10);
    ...
    for (let dx = -w; dx <= w; dx++) {
      for (let dz = -d; dz <= d; dz++) {
        ...
        if (ev > 1) continue;
        if (Math.random() < 0.15) continue; // make ragged
